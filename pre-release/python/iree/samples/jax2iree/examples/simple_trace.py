...
def trace_and_compile():
  builder = ir_builder.Builder()
  out_avals = tracer.trace_flat_function(
      # Trace function.
      fabs,
      builder=builder,
      in_avals=[
...
def load_and_run(binary):
  compiled = ir_builder.Builder.load_compiled_binary(binary)
  ...
  return compiled.fabs(
      np.asarray([[0.5, -0.5, 3.5, 2.0]]),
...
total_time = timeit.timeit(
    lambda: load_and_run(bytes(original_binary)), number=20)
