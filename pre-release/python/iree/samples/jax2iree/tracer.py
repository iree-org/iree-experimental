...
"""Implements a JAX tracer to import into an MLIR module.

This is just a prototype of one possible design option being debated (the
other being to implement a JAXPR interpreter).
"""
...
from .util.ir_builder import Builder, FunctionBuilder
...
class IreeTrace(core.Trace):
  ...
  def process_call(self, call_primitive, f, tracers, params):
    ...
    avals = tuple(t.aval for t in tracers)
    f = interpret_function_ir(f, self.main, avals, *vals)
    vals_out = f.call_wrapped(*vals)
    return [IreeTracer(self, v, a) for v, a in vals_out]
