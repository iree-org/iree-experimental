...
//   ssh ${USER}@${A100_MACHINE_IP} "/usr/local/cuda/bin/nsys profile --stats=true ~/iree-run-module --function=fill_matmul_static --device=cuda --module=foo.vmfb --input=3452x2044xf32=1 --input=2044x1020xf32=1 --input=3452x1020xf32=1 2>&1"
// ```
//
...
^bb1(%variant_op: !pdl.operation):
  ...
  // fold_tensor_subsets + tiling_canonicalization are expected to properly
  // replace fill + pad by just pad.
  transform.iree.apply_patterns %variant_op 
    {canonicalization, cse, licm, fold_tensor_subsets, tiling_canonicalization}
      : (!pdl.operation) -> ()

  // Warning: this is phase-ordered: fold_tensor_subsets must happen before we
  // materialize the copy. 
  %insert_slice_back = transform.structured.match ops{["tensor.parallel_insert_slice"]} in %variant_op
    : (!pdl.operation) -> !pdl.operation
  ...
  // pad_res_hoisted should have been tracked to the replacing fill op but this
  // only prints that the Listener failed.
  // We therefore have to rematch.
  // transform.print %pad_res_hoisted : !pdl.operation
  %fused_fill = transform.structured.match ops{["linalg.fill"]} in %forall_l1
    : (!pdl.operation) -> !pdl.operation
  // transform.print %fused_fill : !pdl.operation
  // The fill is mapped in step 5, together with the contract that it emulates. 
  ...
  %forall_fill, %tiled_fill = 
    transform.structured.tile_to_forall_op %fused_fill num_threads [2, 2]
      ( mapping = [#gpu.warp<y>, #gpu.warp<x>] )
