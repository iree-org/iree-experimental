...
def main(args):
  ...
  with open(os.path.join(output_dir, "mnist_train.mlir"), "wb") as f:
    Program.get_mlir_module(staged_module).operation.print(f, binary=True)
...
def build_model():
  ...
  class MnistModule(Program):
    ...
    def set_opt_state(mdl, new_opt_state=like(opt_state)):
      mdl._opt_state = new_opt_state

    def initialize(mdl, rng=like(rng)):
      mdl._opt_state = mdl._initialize_optimizer(rng)

    def update(mdl, batch=like(example_batch)):
      new_opt_state = mdl._update_step(batch, mdl._opt_state)
      mdl._opt_state = new_opt_state

    def predict(mdl, inputs=like(example_batch[0])):
